            ...
            <div class="col-12 ">
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo '<p class= "b_id card-text">รหัสหนังสือ: ' . $row["b_id"] . '</p>';
                        echo '<p class="b_name card-text">ชื่อหนังสือ:' . $row["b_name"] . '</p>';
                        echo '<p class="b_write card-text">ผู้แต่ง: ' . $row["b_write"] . '</p>';
                        echo '<p class="b_category card-text">หมวดหมู่: ' . $row["b_category"] . '</p>';
                        echo '<p class="b_price card-text">ราคา: ' . $row["b_price"] . ' บาท</p>';
            ...
            <script>
                ...
                function search() {
                    ...
                    for (var i = 0; i < cards.length; i++) {
                        var card = cards[i];
                        var bookCode = card.querySelector('.b_write').innerText; // เลือกข้อมูลรหัสหนังสือใน card

                        if (bookCode.includes(searchInputValue)) {
